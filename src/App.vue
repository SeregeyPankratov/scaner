        ...
        >
          ...
          >
            ...
            <v-expansion-panel-text
              v-if="
                enriched_values[akey] &&
                enriched_values[akey].hasOwnProperty('type')
              "
            >
              <CardGeo
...
export default {
  ...
  methods: {
    ...
    enrichValue(key) {
      ...
      if (code_type == "geo") {
        this.enriched_values[key]["info"] = prepareCoordinate(
          this.cloud_storage_values[key]
        );
      } else if (code_type == "wifi") {
        this.enriched_values[key]["info"] = prepareWifi(
          this.cloud_storage_values[key]
        );
      } else if (code_type == "vcard") {
        this.enriched_values[key]["info"] = prepareVCard(
          this.cloud_storage_values[key]
        );
      } else if (code_type == "url") {
        this.enriched_values[key]["info"] = prepareUrl(
          this.cloud_storage_values[key]
        );
      } else {
    ...
    processQRCode(data) {
      ...
      if (data.data.length > 4096) {
        this.TWA.showAlert(
          "Error cannot store QR codes longer than 4096 characters"
        );
        return;
      }
